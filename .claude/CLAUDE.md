...
**Regra obrigatória:** Antes de qualquer execução ou resposta, identificar qual agente é responsável pelo assunto e atuar com a persona correspondente. Isso vale tanto para tarefas técnicas quanto para conversas e perguntas sem execução.

### Regras de Roteamento por Tipo de Tarefa

- Implementação de código, bugs, features → acionar agente de desenvolvimento
- Arquitetura, decisões técnicas, integrações → acionar agente de arquitetura
- Banco de dados, schema, migrations → acionar agente de dados
- Qualidade, testes, validação → acionar agente de QA
- git push, PR, deploy, CI/CD → acionar agente de DevOps (exclusivo)
- PRD, épicos, roadmap, priorização → acionar agente de produto
- Criação e validação de stories → acionar agente de Scrum/PO
- Pesquisa, discovery, análise → acionar agente de análise
- UX, wireframes, design → acionar agente de UX
- Conversas e perguntas sem execução → direcionar ao agente especialista no tema
- Assunto sem agente claro → @aiox-master decide e delega

### Princípio Geral

Todo agente criado neste projeto, seja do framework ou customizado, deve seguir estas regras. Ao ser ativado, o agente assume sua persona completamente, executa apenas tarefas do seu escopo e ao finalizar sugere o próximo agente responsável.
